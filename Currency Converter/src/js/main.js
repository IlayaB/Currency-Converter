...
//API KEY: f9ac370a953dc10621fbabee
    ...
    if (window.navigator.onLine) {
        if (activeBtnInsert !== event.target.textContent) {
            ...
            if (activeBtnInsert !== activeBtnOut && insertInput.value != '') {
                fetch(`https://v6.exchangerate-api.com/v6/f9ac370a953dc10621fbabee/latest/${activeBtnInsert}`)
                    .then(response => response.json())
                    .then(data => {
                        updateInsertNotification(data.conversion_rates[activeBtnOut]);
                    })
                fetch(`https://v6.exchangerate-api.com/v6/f9ac370a953dc10621fbabee/latest/${activeBtnOut}`)
                    .then(response => response.json())
                    .then(data => {
